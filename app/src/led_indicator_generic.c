...
static void raise_led_changed_event(struct k_work *_work) {
    ZMK_EVENT_RAISE(
        new_zmk_led_indicator_changed((struct zmk_led_indicator_changed){.leds = zmk_leds_get_current_flags()}));
}
...
void zmk_leds_process_report(struct zmk_hid_led_report_body *report, enum zmk_endpoint endpoint,
    ...
    LOG_DBG("Update LED indicator: endpoint=%d, profile=%d, flags=%x", endpoint, profile, leds);
}